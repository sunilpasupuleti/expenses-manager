...
module.exports = {
  ...
  /**
   * Fetch Recurring Transactions (both income & expense) for all linked accounts
   */
  async getAllRecurringTransactions(req, res) {
    try {
      const { uid } = req.user;

      // 🔁 Fetch user's access tokens from Firebase
      const snapshot = await database()
        .ref(`/users/${uid}/plaid/accessTokens`)
        .once("value");
      const accessTokens = snapshot.val() || [];
      ...
      const sortByNextDate = (a, b) => {
        if (!a.predictedNextDate) return 1;
        if (!b.predictedNextDate) return -1;
        return a.predictedNextDate.localeCompare(b.predictedNextDate);
      };

      inflowActive.sort(sortByNextDate);
      inflowInactive.sort(sortByNextDate);
      outflowActive.sort(sortByNextDate);
      outflowInactive.sort(sortByNextDate);

      const finalData = {
        inflow: {
          active: inflowActive,
          inactive: inflowInactive,
        },
        outflow: {
          active: outflowActive,
          inactive: outflowInactive,
        },
      };
      ...
      console.error("Error fetching recurring transactions:", errorMessage);
      return sendResponse(res, httpCodes.INTERNAL_SERVER_ERROR, {
        message: errorMessage,
      });
    }
